...
func TestPublicKey_MultPlain(t *testing.T) {
	pk, sk, err := GenerateKeyPair(1024)
	if err != nil {
		t.Errorf("Error generating key pair")
		return
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := pk.MultPlain(tt.args.ct, tt.args.pt)
			if (err != nil) != tt.wantErr {
				t.Errorf("PublicKey.MultPlain() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if err != nil {
				return
			...
			sum, err := sk.Decrypt(got)
			if err != nil {
				t.Errorf("PublicKey.MultPlain() error = invalid ciphertext generated by addition")
				return
			}
			if sum != tt.want {
				t.Errorf("PublicKey.MultPlain() = %v, want %v", sum, tt.want)
			}
		})
	}
}
